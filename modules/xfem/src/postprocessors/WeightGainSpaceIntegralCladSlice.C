...
#include "WeightGainSpaceIntegralCladSlice.h"

registerMooseObject("MooseApp", WeightGainSpaceIntegralCladSlice);

InputParameters
WeightGainSpaceIntegralCladSlice::validParams()
{
  ...
  params.addParam<Real>("angle", 360, "The angle of the slice");
...
WeightGainSpaceIntegralCladSlice::WeightGainSpaceIntegralCladSlice(const InputParameters & parameters)
...
void
WeightGainSpaceIntegralCladSlice::initialize()
{
}

void
WeightGainSpaceIntegralCladSlice::execute()
{
  ...
  // y)
  Real full_weak_integral = _C_integral / _zmax * Czr * 1e-6; ///!!!units!!!
  ...
  //need for oxide layer and alpha layer mean radius 
  Real R_ox_avg = _x_ox_a + 0.5 * _delta;
  Real R_a_avg = _x_a_b + 0.5 * _d_alpha;
  // can now get the oxide part of the weak integral [/m²]:
  Real oxide_weak_integral =
      1e-6 * (_delta * 2 * pi * _angle / 360 * R_ox_avg ) / Zr_PBR * (C_a_ox - (C_a_b - C_b_a) + 0.5 * (C_ox - C_ox_a));
  ...
  // [/m²]:
  Real metal_strong_missing = 1e-6 * (_d_alpha * 2 * pi * _angle / 360 * R_a_avg ) * (C_a_b - C_b_a);

  // The integral in the oxide (linear concentration) [/m²]:
  Real oxide_strong_integral = 1e-6 * (_delta * 2 * pi * _angle / 360 * R_ox_avg ) * 0.5 * (C_ox + C_ox_a);
  ...
  Real metal_native_integral = 1e-6 * pi * (pow(2000,2) - pow(1400,2)) * C_nat;
...
Real
WeightGainSpaceIntegralCladSlice::getValue()
{
  return _wg;
}
