...
#include "XFEMEqualValueAtInterfaceC4aoxOld.h"
#include "FEProblem.h"
#include "GeometricCutUserObject.h"
#include "XFEM.h"

registerMooseObject("XFEMApp", XFEMEqualValueAtInterfaceC4aoxOld);

InputParameters
XFEMEqualValueAtInterfaceC4aoxOld::validParams()
{
...
XFEMEqualValueAtInterfaceC4aoxOld::XFEMEqualValueAtInterfaceC4aoxOld(const InputParameters & parameters)
    ...
    _offset(getParam<bool>("offset"))
{
  ...
  if (_xfem == nullptr)
    mooseError("Problem casting to XFEM in XFEMEqualValueAtInterfaceC4aoxOld");
  ...
  if (dynamic_cast<const GeometricCutUserObject *>(uo) == nullptr)
    mooseError("UserObject casting to GeometricCutUserObject in XFEMEqualValueAtInterfaceC4aoxOld");
  ...
  if (_offset)
  {
    ...
    _value = _value; //- (c_o_a_b - c_o_b_a); One interface only
  }


}

XFEMEqualValueAtInterfaceC4aoxOld::~XFEMEqualValueAtInterfaceC4aoxOld() {}

void
XFEMEqualValueAtInterfaceC4aoxOld::reinitConstraintQuadrature(const ElementPairInfo & element_pair_info)
{
  ElemElemConstraint::reinitConstraintQuadrature(element_pair_info);
}

Real
XFEMEqualValueAtInterfaceC4aoxOld::computeQpResidual(Moose::DGResidualType type)
{
  Real r = 0;
...
Real
XFEMEqualValueAtInterfaceC4aoxOld::computeQpJacobian(Moose::DGJacobianType type)
{
  Real r = 0;
