...
#include "C4ZrIC1Int.h"
#include "MooseUtils.h"

registerMooseObject("MooseApp", C4ZrIC1Int);

InputParameters
C4ZrIC1Int::validParams()
{
...
C4ZrIC1Int::C4ZrIC1Int(const InputParameters & parameters)
  : InitialCondition(parameters), _temperature(getParam<Real>("temperature"))
{
  ...
  else
  {
    ...
    _x_ox_a = -3.6071 * 1e-5 * pow(_temperature, 2) + 7.1427 * 1e-2 * _temperature + 563.11+1400;
    ...
    _x_a_break = -1.5750 * 1e-4 * pow(_temperature, 2) + 0.37183 * _temperature + 375.50+1400;
  ...
  _grad_ox = (_C_ox_weak - _C_ox_a_weak) / (2000 - _x_ox_a);
}

Real
C4ZrIC1Int::value(const Point & p)
{
