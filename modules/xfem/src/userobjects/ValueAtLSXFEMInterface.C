...
#include "ValueAtLSXFEMInterface.h"
...
registerMooseObject("XFEMApp", ValueAtLSXFEMInterface);

InputParameters
ValueAtLSXFEMInterface::validParams()
{
  ...
  params.addParam<UserObjectName>(
      ...
      "Name of LevelSetCutUserObject that provides cut locations to this UserObject.");
  params.addRequiredParam<VariableName>(
  ...
  //class descritption
  params.addClassDescription("Obtain field values and gradients on the interface for a levelset cut. Only a test.");
  return params;
}

ValueAtLSXFEMInterface::ValueAtLSXFEMInterface(const InputParameters & parameters)
...
void
ValueAtLSXFEMInterface::initialize()
{
  _pl = _mesh.getPointLocator();
  _xfem = MooseSharedNamespace::dynamic_pointer_cast<XFEM>(_fe_problem.getXFEM());
  if (_xfem == nullptr)
    mooseError("Problem casting to XFEM in ValueAtLSXFEMInterface");

  const UserObject * uo =
      &(_fe_problem.getUserObjectBase(getParam<UserObjectName>("level_set_cut_userobject")));

  if (dynamic_cast<const LevelSetCutUserObject *>(uo) == nullptr)
    mooseError("UserObject casting to LevelSetCutUserObject in ValueAtLSXFEMInterface");

  
  _ls_cut = dynamic_cast<const LevelSetCutUserObject *>(uo);
  _elem_pairs = _xfem->getXFEMCutElemPairs(_xfem->getGeometricCutID(_ls_cut));

}

void
ValueAtLSXFEMInterface::execute()
{
...
void
ValueAtLSXFEMInterface::finalize()
{
...
/*Point
ValueAtLSXFEMInterface::getPointCurrentLocation(unsigned int i) const
{
  return _nodes[i];
};
*/
const Elem *
ValueAtLSXFEMInterface::getElemContainingPoint(const Node & p, bool positive_level_set)
{
  ...
  if (!found)
    mooseError("ValueAtLSXFEMInterface: The interface node ",
               p,
