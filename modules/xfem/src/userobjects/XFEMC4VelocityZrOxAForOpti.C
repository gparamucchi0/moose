...
registerMooseObject("XFEMApp", XFEMC4VelocityZrOxAForOpti);

InputParameters
XFEMC4VelocityZrOxAForOpti::validParams()
{
...
XFEMC4VelocityZrOxAForOpti::XFEMC4VelocityZrOxAForOpti(const InputParameters & parameters)
...
//  std::cout << "xt: " << xt << std::endl;
  ...
  const Real x_o_ox_m = 0.667118123 - 1.10606e-5 * _temperature; //the original one, not the weak equivalent
  ...
  const Real J_o = -_diffusivity_alpha * con_zr * (-grad_negative * 1e-6) * normal;
