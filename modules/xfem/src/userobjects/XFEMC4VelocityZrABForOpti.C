...
InputParameters
XFEMC4VelocityZrABForOpti::validParams()
{
...
Real
XFEMC4VelocityZrABForOpti::computeMovingInterfaceVelocity(dof_id_type point_id, RealVectorValue normal) const
{
...
//  Values at the interface (strong discontinuity)
  ...
  const Real J_b_to_a = -_diffusivity_alpha * grad_positive * normal;
  const Real J_a_to_b = -_diffusivity_beta * (-grad_negative) * normal;
